...
def test_blocked_finding_catalog_covers_v0296_withdrawal_conditions() -> None:
    blocked = ProviderInvocationApprovalAuditRollbackFindingService.BLOCKED_FINDINGS
...
def test_report_never_marks_live_runtime_or_approval_readiness() -> None:
    report = ProviderInvocationApprovalAuditRollbackReportService().build_report()
    ...
    assert report.approval_audit_rollback_gate.no_approval_granted is True
    assert report.approval_audit_rollback_gate.no_provider_invocation is True
    assert report.approval_audit_rollback_gate.no_network_call is True
    assert report.approval_audit_rollback_gate.no_credential_access is True
    assert report.approval_audit_rollback_gate.no_command_execution is True
    assert report.approval_audit_rollback_gate.no_external_side_effect is True
    assert report.approval_audit_rollback_gate.no_rollback_execution is True
...
def test_v0296_changed_files_do_not_add_forbidden_runtime_surfaces() -> None:
    root = Path(__file__).resolve().parents[1]
    changed_files = [
        root / "src" / "chanta_core" / "provider_invocation_approval_audit_rollback_boundary.py",
        root / "src" / "chanta_core" / "cli" / "main.py",
        root
        / "docs"
        / "versions"
        / "v0.29"
        / "v0.29.6_provider_invocation_approval_audit_rollback_boundary.md",
        root / "tests" / "test_provider_invocation_approval_audit_rollback_boundary.py",
        root / "tests" / "test_provider_invocation_approval_audit_rollback_boundary_boundaries.py",
    ]
