...
def test_blocked_finding_catalog_covers_v0295_withdrawal_conditions() -> None:
    blocked = AdapterInvocationCandidateFindingService.BLOCKED_FINDINGS
...
def test_report_never_marks_live_runtime_readiness() -> None:
    report = AdapterInvocationCandidateReportService().build_report()
    ...
    assert report.readiness_gate.no_provider_invocation is True
    assert report.readiness_gate.no_network_call is True
    assert report.readiness_gate.no_credential_access is True
    assert report.readiness_gate.no_command_execution is True
    assert report.readiness_gate.no_external_side_effect is True
...
def test_v0295_changed_files_do_not_add_forbidden_runtime_surfaces() -> None:
    root = Path(__file__).resolve().parents[1]
    changed_files = [
        root / "src" / "chanta_core" / "adapter_invocation_candidate_dry_run_plan.py",
        root / "src" / "chanta_core" / "cli" / "main.py",
        root / "docs" / "versions" / "v0.29" / "v0.29.5_adapter_invocation_candidate_dry_run_plan.md",
        root / "tests" / "test_adapter_invocation_candidate_dry_run_plan.py",
        root / "tests" / "test_adapter_invocation_candidate_dry_run_plan_boundaries.py",
    ]
