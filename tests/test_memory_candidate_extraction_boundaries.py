from __future__ import annotations

from pathlib import Path

from chanta_core.cli.main import main
from chanta_core.memory_candidate_continuity import (
    MemoryCandidateExtractionFindingService,
    MemoryCandidateExtractionReportService,
    MemoryCandidateExtractionRuleService,
    MemoryCandidateExtractionDecisionService,
    MemorySourceRefService,
)


def test_missing_source_boundary_warns_without_scoring_or_promotion() -> None:
    report = MemoryCandidateExtractionReportService().build_all_parts(source_boundary_available=False)["report"]
    finding_types = {finding.finding_type for finding in report.findings}

    assert report.report_status == "warning"
    assert report.ready_for_v0_27_3 is False
    assert "missing_source_boundary_report" in finding_types
    assert "missing_candidate_readiness_boundary" in finding_types
    assert "missing_eligible_sources" in finding_types
    assert report.memory_candidate_count == 0
    assert report.memory_scored is False
    assert report.memory_promoted is False
    assert report.persistent_memory_written is False
    assert report.durable_memory_written is False
    assert report.session_continuity_injected is False
    assert report.persona_mutated is False
    assert report.behavior_policy_mutated is False
    assert report.provider_invoked is False
    assert report.command_executed is False


def test_raw_source_decision_blocks_candidate_creation() -> None:
    source = MemorySourceRefService().build_source_refs()[0]
    source.raw_content_included = True
    source.raw_transcript_included = True
    decision = MemoryCandidateExtractionDecisionService().decide_extraction(
        source,
        "memory_candidate:v0.27.2:blocked",
        MemoryCandidateExtractionRuleService().build_rules(),
    )

    assert decision.decision_type == "block_raw_source"
    assert decision.creates_memory_candidate is False
    assert decision.creates_memory is False
    assert decision.scores_memory is False
    assert decision.promotes_memory is False
    assert decision.writes_persistent_memory is False


def test_forbidden_attempt_findings_block_candidate_extraction() -> None:
    service = MemoryCandidateExtractionReportService()
    for finding_type in MemoryCandidateExtractionFindingService.BLOCKED_FINDINGS:
        report = service.build_all_parts(extra_findings=[finding_type])["report"]
        finding_types = {finding.finding_type for finding in report.findings}

        assert report.report_status == "blocked"
        assert report.ready_for_v0_27_3 is False
        assert finding_type in finding_types


def test_memory_candidate_cli_commands_are_extraction_only(capsys) -> None:
    commands = [
        ["memory", "candidates", "extract"],
        ["memory", "candidates", "source-view"],
        ["memory", "candidates", "rules"],
        ["memory", "candidates", "classifier"],
        ["memory", "candidates", "batch"],
        ["memory", "candidates", "list"],
        ["memory", "candidates", "claims"],
        ["memory", "candidates", "source-links"],
        ["memory", "candidates", "context"],
        ["memory", "candidates", "provenance"],
        ["memory", "candidates", "pig-signals"],
        ["memory", "candidates", "risks"],
        ["memory", "candidates", "decisions"],
        ["memory", "candidates", "audit"],
        ["memory", "candidates", "report"],
    ]

    for command in commands:
        assert main(command) == 0
        output = capsys.readouterr().out
        assert "version=v0.27.2" in output
        assert "extraction_policy_created=true" in output
        assert "extraction_source_view_created=true" in output
        assert "candidate_batch_created=true" in output
        assert "memory_candidates_created=true" in output
        assert "ready_for_v0_27_3=true" in output
        assert "ready_for_v0_28=false" in output
        assert "memory_scored=false" in output
        assert "memory_promoted=false" in output
        assert "persistent_memory_written=false" in output
        assert "durable_memory_written=false" in output
        assert "session_continuity_injected=false" in output
        assert "persona_mutated=false" in output
        assert "behavior_policy_mutated=false" in output
        assert "raw_transcript_memory_created=false" in output
        assert "raw_provider_output_memory_created=false" in output
        assert "pig_memory_promoted=false" in output
        assert "pig_policy_mutated=false" in output
        assert "pig_executed=false" in output
        assert "provider_invoked=false" in output
        assert "command_executed=false" in output
        assert "safety_gate_bypassed=false" in output
        assert "external_provider_adapter_implemented=false" in output
        assert "schumpeter_split_introduced=false" in output
        assert "raw_secret_output=false" in output
        assert "credential_exposed=false" in output
        assert "llm_judge_used=false" in output


def test_v0272_changed_files_do_not_contain_forbidden_true_markers() -> None:
    root = Path(__file__).resolve().parents[1]
    paths = [
        root / "src" / "chanta_core" / "memory_candidate_continuity.py",
        root / "tests" / "test_memory_candidate_extraction.py",
        root / "tests" / "test_memory_candidate_extraction_boundaries.py",
        root / "docs" / "versions" / "v0.27" / "v0.27.2_memory_candidate_extraction.md",
    ]
    forbidden_names = [
        "memory_candidate_scored",
        "memory_scored",
        "memory_promoted",
        "persistent_memory_written",
        "durable_memory_written",
        "durable_memory_record_created",
        "durable_memory_registry_updated",
        "session_continuity_context_created",
        "continuity_injection_bundle_created",
        "persona_mutated",
        "behavior_policy_auto_mutated",
        "behavior_policy_mutated",
        "raw_transcript_persisted_as_memory",
        "raw_provider_output_persisted_as_memory",
        "raw_transcript_memory_created",
        "raw_provider_output_memory_created",
        "pig_memory_promoted",
        "pig_policy_mutated",
        "pig_executed",
        "provider_invoked",
        "command_executed",
        "safety_gate_bypassed",
        "safety_gate_bypassed_by_memory",
        "external_provider_adapter_implemented",
        "external_agent_adapter_implemented",
        "schumpeter_split_introduced",
        "raw_secret_output",
        "credential_exposed",
    ]
    forbidden_runtime_markers = [
        "subprocess" + ".run",
        "subprocess" + ".Popen",
        "os" + ".system",
        "shell" + "=True",
        "open" + "ai",
        "anth" + "ropic",
        "chat" + "." + "com" + "pletions",
        "exec" + "(",
        "eval" + "(",
    ]

    for path in paths:
        text = path.read_text(encoding="utf-8")
        for name in forbidden_names:
            assert f"{name}=True" not in text
        for marker in forbidden_runtime_markers:
            assert marker not in text
