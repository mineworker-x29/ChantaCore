...
from chanta_core.permission_safety_scope_gate_for_external_adapters import (
    AdapterPermissionSafetyFindingService,
    AdapterPermissionSafetyReportService,
)


def test_v0293_blocked_finding_types_cover_permission_safety_drift() -> None:
    blocked = AdapterPermissionSafetyFindingService.BLOCKED_FINDINGS
...
def test_v0293_report_forbidden_flags_remain_false() -> None:
    report = AdapterPermissionSafetyReportService().build_report()

    forbidden_flags = [
        "ready_for_invocation_candidate",
    ...
    assert report.policy.permission_gate_is_not_permission_grant is True
    assert report.policy.safety_gate_is_not_invocation is True
    assert report.permission_safety_gate.no_permission_granted is True
    assert report.permission_safety_gate.no_provider_invocation is True
    assert report.permission_safety_gate.no_network_call is True
    assert report.permission_safety_gate.no_credential_storage is True
    assert report.permission_safety_gate.no_command_execution is True


def test_v0293_changed_files_do_not_encode_forbidden_true_flags_or_execution_calls() -> None:
    root = Path(__file__).resolve().parents[1]
    paths = [
        root / "src" / "chanta_core" / "permission_safety_scope_gate_for_external_adapters.py",
        root / "src" / "chanta_core" / "cli" / "main.py",
        root / "tests" / "test_permission_safety_scope_gate_for_external_adapters.py",
        root / "tests" / "test_permission_safety_scope_gate_for_external_adapters_boundaries.py",
        root / "docs" / "versions" / "v0.29" / "v0.29.3_permission_safety_scope_gate_for_external_adapters.md",
    ]
