...
def test_chat_and_capability_self_report_available_now(tmp_path) -> None:
    service, surface = _build("what can you do", tmp_path)
...
def test_workspace_read_not_available_now(tmp_path) -> None:
    service, surface = _build("/PersonalDirectory/sample_profile.md read markdown", tmp_path)
...
def test_shell_network_mcp_plugin_are_not_available_now(tmp_path) -> None:
    prompts = [
        ("run powershell command", "requires_permission"),
        ("call https://example.com", "requires_permission"),
        ("connect MCP server", "not_implemented"),
        ("load plugin", "not_implemented"),
    ]
...
def test_external_candidates_are_disabled_or_review_required(tmp_path) -> None:
    service, _ = _build("external capability use", tmp_path)
...
def test_render_decision_surface_block_guides_limitations(tmp_path) -> None:
    service, surface = _build("run powershell command", tmp_path)
