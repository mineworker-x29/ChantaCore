...
def test_v0291_blocked_finding_types_cover_registry_drift() -> None:
    blocked = AdapterRegistryFindingService.BLOCKED_FINDINGS
...
def test_v0291_report_forbidden_flags_remain_false() -> None:
    report = AdapterRegistryReportService().build_report()
    ...
    assert report.inventory_policy.inventory_enabled is True
    assert report.registry_policy.registry_entries_are_metadata_only is True
    assert report.registry_gate.no_provider_registration is True
    assert report.registry_gate.no_provider_invocation is True
    assert report.registry_gate.no_network_call is True
    assert report.registry_gate.no_credential_storage is True
    assert report.registry_gate.no_command_execution is True


def test_v0291_changed_files_do_not_encode_forbidden_true_flags_or_execution_calls() -> None:
    root = Path(__file__).resolve().parents[1]
    paths = [
        root / "src" / "chanta_core" / "provider_capability_inventory_adapter_registry.py",
        root / "src" / "chanta_core" / "cli" / "main.py",
        root / "tests" / "test_provider_capability_inventory_adapter_registry.py",
        root / "tests" / "test_provider_capability_inventory_adapter_registry_boundaries.py",
        root / "docs" / "versions" / "v0.29" / "v0.29.1_provider_capability_inventory_adapter_registry.md",
    ]
