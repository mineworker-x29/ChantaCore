...
def test_blocked_finding_catalog_covers_v0297_withdrawal_conditions() -> None:
    blocked = AdapterPackagingCertificationFindingService.BLOCKED_FINDINGS
...
def test_report_never_marks_publish_live_or_runtime_readiness() -> None:
    report = AdapterPackagingCertificationReportService().build_report()
    ...
    assert report.certification_readiness_gate.no_package_publish is True
    assert report.certification_readiness_gate.no_release_tag is True
    assert report.certification_readiness_gate.no_live_certification is True
    assert report.certification_readiness_gate.no_provider_invocation is True
    assert report.certification_readiness_gate.no_network_call is True
    assert report.certification_readiness_gate.no_credential_access is True
    assert report.certification_readiness_gate.no_command_execution is True
...
def test_v0297_changed_files_do_not_add_forbidden_runtime_surfaces() -> None:
    root = Path(__file__).resolve().parents[1]
    changed_files = [
        root / "src" / "chanta_core" / "external_skill_packaging_certification_matrix.py",
        root / "src" / "chanta_core" / "cli" / "main.py",
        root / "docs" / "versions" / "v0.29" / "v0.29.7_external_skill_packaging_certification_matrix.md",
        root / "tests" / "test_external_skill_packaging_certification_matrix.py",
        root / "tests" / "test_external_skill_packaging_certification_matrix_boundaries.py",
    ]
